...
class IndexBasicCollector(BaseCollector):
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        for market in markets:
            try:
                market_records, source = await data_source_manager.get_index_basic(market=market)
                if market_records:
                    records.extend(market_records)
                    self.logger.info(f"Fetched {len(market_records)} indices from {market} via {source}")
